...
**流程图（研究思路/模型结构/求解流程）一律按本队模板标准绘制**：绘图前必读 `<SKILL_ROOT>/references/流程图/流程图学习笔记.md`，并对照模板图（`<SKILL_ROOT>/references/流程图/flow_2~6.png`）执行——浅色分区块 + 一个主色相家族 + Problem/Task/Step 编号导航 + 短标签文字 + 三档字号层级；**禁止**纯白底无分区色块的"草稿式"流程图（教训见笔记反例对照节）；**结论性文字与解释性注释尽量放正文，不放图内**（2019 C 题教训：图例注释与坐标重叠、图内放结论均被要求移除）。
...
**正文写作应使用连贯的学术段落。论文是给评委看的，不是工作记录**——最终论文中不得出现任何工作痕迹：工作流内部名称（`reports/`、`figures/`、`CLAUDE.md`、skill 名等）、解题手分工（"我负责 Q3"之类）、AI 调用/对话/求解过程、门禁与质检记录；论文只呈现题目、模型、求解、结果与结论。写完后按此清单自查一遍：①全文是否有上述工作痕迹字样；②图表标题、正文是否有 `→`、`✓` 等非书面符号；③表述是否机械（连段论述而非条目堆砌）——任一命中即整改。

**Word 形态论文（docx）一律以超级模板为底生成**：写作/草稿用 Word 时，复制 `<SKILL_ROOT>/tools/docx/templates/数学建模论文超级模板.docx` → 改文件名 → 按 `<SKILL_ROOT>/references/论文结构超级模板.md` 逐节替换内容（模板内写作经验已提炼入该指南）→ 插图（详见 `<SKILL_ROOT>/tools/docx/SKILL.md`"论文 Word 生成"节）。
...
- 工作形态：Word 草稿（docx，以超级模板为底复制生成，见步骤 4）或 Typst/LaTeX 源码。所有修改都在源码/草稿上进行，未确认终稿前不执行最终编译。
...
1. **主 Agent 盘点候选文件**：全部可运行源程序（含 EXCEL、SPSS 等交互命令）、自主查阅使用的数据资料（赛题提供的原始数据除外）、较大篇幅中间结果的图表等；对应功能缺失时明确说明（如"本队未使用额外数据资料"）。**排除项**：AI 生成的思路类文档（思路总结、题目分析报告、术语对照表等 md/pdf）一律不放入——支撑材料是给评委看的，不是队内工作档案；重复内容只保留一份（如同一数据在文件夹与压缩包中选一处即可）。
