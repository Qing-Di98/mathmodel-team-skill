...
**当届官方格式终检（国赛 2026 修订稿，硬性）**：按 `<SKILL_ROOT>/references/论文格式规范2026.md` 与 `竞赛要求.md` 检查并写入 `VERIFY_REPORT.md`——纸质版：摘要页（摘要含标题和关键词 ≤1 页、从摘要页起页码 1 开始连续编号）、正文无目录 ≤30 页、附录含支撑材料文件列表与全部完整可运行源程序、全文无参赛者身份/学校/赛区信息、参考文献按科技论文规范标注；**电子版**：单独一个文件（PDF 或 Word 之一）、与纸质版内容格式（含附录）完全一致、≤20MB、未压缩、**第一页必须为摘要专用页（不含承诺书和编号专用页）**；支撑材料：**生成前先给用户过目文件清单并经用户确认（见 `5writing` 步骤 8）**、单独压缩文件（WinRAR，RAR/ZIP，≤20MB）、条目数与已确认清单一致、文件列表在附录、无支撑材料须注明"本论文没有支撑材料"。任何一项不满足即为硬错误，不得声称交付就绪。
...
**终稿前提**：论文必然反复修改，PDF 仅在用户明确确认终稿后生成（见 `5writing` 步骤 7）。用户未确认终稿时，本阶段只做源码/草稿级检查（文本质量、结构、图表引用、数值一致性、编译日志），不做 PDF 逐页视觉检查与最终提交清单判定；用户确认终稿并生成 PDF 后，才执行完整终检（含 PDF 逐页视觉检查与当届电子版要求核验）。
