...
**锚点回收（强制第一步）**：动手求解前，先在建模报告/队友结果中确认本问锚点（解析基准公式、队友已验数值、理论值）；实现后先跑**锚点回收**——用最小用例验证与解析基准/已知结果一致（如误差 ≤10%），全部通过后才允许跑正式方案并输出正式结果；锚点不过先修口径再继续（2019 C 题教训：Q4 以 $\alpha$=1.0 复现 Q1/Q2 的 $E[W|n=20]$=16.75 min、$n^*$=145 等锚点，3/3 通过才认定仿真口径正确）。
