...
## 论文 Word 生成（超级模板优先，本队默认流程）

**用户指令（2026-08-16，原话）："以后生成论文 word 就先复制数学建模论文超级模板.docx，然后改文件名、改内容、插图即可"。**

生成/新建 Word 论文一律按此流程：

1. 复制 `<SKILL_ROOT>/tools/docx/templates/数学建模论文超级模板.docx` 到论文项目根目录。
2. 改文件名（如 `论文.docx`），不重新生成 Word——模板自带完整样式（标题层级/编号/字体/表格），复制后只改内容。
3. 按 `<SKILL_ROOT>/references/论文结构超级模板.md` 逐节替换内容（删除模板内嵌指导文字），该指南提炼了模板内全部写作经验。
4. 插图：按 5writing 图表规划；已排好版需要换图时用下节"Word 原位替换图片"改包，不重新生成。

> 仓库中另有上游遗留骨架 `references/roles/论文手/references/论文模板.docx`（无内嵌写作经验，仅供旧角色档案参考）——**Word 论文一律用超级模板，不用旧骨架**。
> 当届官方模板（`论文格式规范2026.md` 要求的严格格式）场景仍可用下方"数学建模论文推荐流程"的 python-docx 构建方案，两种流程产出均须通过"必做验证"。

## 数学建模论文推荐流程（官方模板严格场景备选）

采用“当届官方参考模板 + `python-docx` 构建 + OMML 公式 + OOXML 校验 + 渲染抽检”。官方模板控制页面、样式、分节、页眉页脚和编号；代码负责稳定写入内容。**默认不启用，仅当用户明确要求按当届官方模板严格构建时使用。**
